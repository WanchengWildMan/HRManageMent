...
void Main::InputReward() {
  ...
  for (int i = 0; i < Peo.size(); i++) {
    ...
    if (Peo[i].jobid == 2) {
      ...
      FinancialModule.GetMoneyByJob(Peo[i], salevolume);
    }
    if (Peo[i].jobid == 3) {
      cout << "工作时间:" << endl;
      double hours;
      ...
      InputModule.InputSaleVolumeOrHours("gong'z");
      FinancialModule.GetMoneyByJob(Peo[i], hours);
    }
    if (Peo[i].jobid == 4) {
      double salevolume = InputModule.InputSaleVolumeOrHours();
      FinancialModule.GetMoneyByJob(Peo[i], salevolume);
    }
  }
}
