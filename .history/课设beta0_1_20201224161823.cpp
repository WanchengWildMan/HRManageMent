...
class Financial : public Function {
  ...
  inline double GetMoneyByJob(Staff &worker,double plusmoneyOrhours) {
    if (worker.jobid == 1) {
      worker.money=MONEY_MANAGER;
    }
    if(worker.jobid==2){
      worker.money = MONEY_SALEMANAGER + plusmoneyOrhours;
      worker.plusmoney = plusmoneyOrhours;
    }
    if(worker.jobid==3){
      worker.hours =plusmoneyOrhours;
      worker.money = worker.hours * MONEY_PER_HOUR;
    }
    if(worker.jobid==4){
      worker.money = plusmoneyOrhours;
      //+销售额?
      worker.plusmoney = plusmoneyO;
      worker.salevolume = salevolume;
    }
  }
} FinancialModule;
...
void Main::InputReward() {
  ...
  for (int i = 0; i < Peo.size(); i++) {
    ...
    double plusmoney = 0;
    if (Peo[i].jobid == 2) {
      double salevolume = inputSaleVolume();
